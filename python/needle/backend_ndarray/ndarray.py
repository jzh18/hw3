...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        for i,s in enumerate(idxs):
            # print(f'idxs: {idxs}')
            offset+=s.start*self._strides[i]
            stride=s.step
            dim_size=int((s.stop-s.start)//stride)
            ...
            array=self.make(new_shape,strides=tuple(new_stride),device=self._device,handle=self._handle,offset=offset)
            self._init(array)
            # print(self)
        return self
        ### END YOUR SOLUTION
